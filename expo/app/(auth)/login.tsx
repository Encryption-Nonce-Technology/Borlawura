...
export default function LoginScreen() {
  ...
  const requestOtp = trpc.auth.requestOtp.useMutation({ onSuccess: (data) => { setOtpRequested(true); Alert.alert("OTP sent", data.codeForDev ? `Development code: ${data.codeForDev}` : "Check your SMS."); }, onError: (error) => Alert.alert("Could not send OTP", error.message) });
  const verifyOtp = trpc.auth.verifyOtp.useMutation({ onSuccess: async (data) => { await saveSession({ token: data.token, user: data.user! as any }); router.replace("/" as any); }, onError: (error) => Alert.alert("Could not sign in", error.message) });
  const pending = requestOtp.isPending || verifyOtp.isPending;
  const submit = () => { if (!otpRequested) requestOtp.mutate({ phone }); else verifyOtp.mutate({ phone, code: otp, role, name: role === "collector" ? "New Collector" : "Borlawura User" }); };

  return <SafeAreaView style={styles.page}><View style={styles.hero}><Image source={require("@/assets/images/icon.png")} style={styles.logo} /><Text style={styles.title}>{Brand.appName}</Text><Text style={styles.subtitle}>Waste pickup made simple.</Text></View><View style={styles.card}><Text style={styles.heading}>{otpRequested ? "Verify your number" : "Sign in"}</Text><Text style={styles.help}>Use your Ghana phone number to securely access your account.</Text><TextInput style={styles.input} value={phone} onChangeText={setPhone} keyboardType="phone-pad" placeholder="+233 24 000 0000" />{!otpRequested && <View style={styles.roleRow}>{(["user", "collector"] as const).map((item) => <Pressable key={item} onPress={() => setRole(item)} style={[styles.role, role === item && styles.roleActive]}><Text style={[styles.roleText, role === item && styles.roleTextActive]}>{item === "user" ? "I need pickup" : "I collect waste"}</Text></Pressable>)}</View>}{otpRequested && <TextInput style={styles.input} value={otp} onChangeText={setOtp} keyboardType="number-pad" placeholder="6-digit OTP" maxLength={6} />}<Pressable style={[styles.button, pending && styles.disabled]} onPress={submit} disabled={pending || phone.length < 10 || (otpRequested && otp.length !== 6)}><Text style={styles.buttonText}>{pending ? "Please wait..." : otpRequested ? "Verify and continue" : "Send OTP"}</Text></Pressable></View></SafeAreaView>;
}
